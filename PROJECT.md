...
- 仿文石 leaf：屏幕外露（前框开窗压边），**左薄**（屏幕+**电池贴屏幕背**）/ **右厚手柄**（只放 PCB+按键，握持）
- **新布局（电池贴屏幕背）**：超薄平板锂电池放屏幕背玻璃（左侧薄区），手柄只留 **100×40mm 主控板** + 按键；ESP32 放 PCB 上部、天线朝上无遮挡（电池/手都不挡）
- **FPC/按键定位**：7.5" 屏竖放时 FPC 从**右缘中部**引出（离屏底约 85mm）→ PCB 垂直放（离底约 5~105mm），FPC 座对齐该引出点，翻页键在 PCB 中下（拇指区，离底 40~70mm）
- 按键：前侧右操作区上下 2 翻页键（拇指）+ 右侧面上部确认/电源键(SW1)；顶部 ESP32 天线开窗；底部 TypeC
- 版本：`shell_v0.scad`（概念 v0.2）→ `shell_v0.3.scad`（新布局，待完善，保留各版便于微调）
- 嘉立创 3D 打印收 **STL**；嘉立创 PCB 免费打样限 **100×100mm** 内（主控板 100×40 达标）

**电池选型（贴屏幕背）**：约 **140×90mm、厚 ≤5mm、容量 ≥2500mAh** 超薄平板聚合物锂电（屏幕背可用 ~160×100mm）。注意：电池与屏幕玻璃间加绝缘缓震垫防穿刺/压坏；引线沿边缘到手柄 PCB。
...
### 2026-09-09（布局定稿：电池贴屏幕背 + 手柄只放PCB）
- **电池移到屏幕背后**（左侧薄区超薄平板锂电），解决"PCB放中下部后电池挤上部、ESP32被夹+手挡"问题
- **手柄只放 100×40 主控板 + 按键**；ESP32 放 PCB 上部、天线朝上无遮挡（电池/手都不挡）
- **定位依据**：7.5" 屏竖放 FPC 从右缘中部(离屏底~85mm)引出；PCB 垂直放(离底~5~105mm)，FPC 座对齐中部引出点、翻页键在拇指区(离底40~70mm)
- 电池选型：约 140×90×≤5mm、≥2500mAh 超薄平板锂电；玻璃间加绝缘缓震
- 外壳 shell_v0.scad 参数待手部实测后精调（用户量手部高度）

### 2026-09-09（DS3231 无后备 + 对时策略 + 分体 PCB 尺寸定稿）
- **DS3231 无 CR2032 后备**：用 3V3(系统 LDO) 供电，VBAT 悬空/GND；系统关机(RTC 断电)→时间丢失
- **对时策略**：每次上电后先做一次 **NTP 联网对时**；若未联网/对时失败，则**提示用户进入设置页手动对时**
- **PCB 分体式**：主控板独立，屏幕不贴 PCB（外壳边框保护），屏幕 FPC 排线连主控
- 主控板尺寸：**100mm(长) × 40mm(宽)**（细长条，元件**双面贴**，放右侧手柄区；长边 100 在嘉立创免费打样限内；精确参数待用户定稿）
